...
function ReactLiveTime(props) {
    ...
    var _c = useState(diffSetter(time)), diff = _c[0], setDiff = _c[1];
    var _d = useState(getStatusFromTime(diff)), status = _d[0], setStatus = _d[1];
    var _e = useState(setCompText(diff, time, status, format)), text = _e[0], setText = _e[1];
    useEffect(function () { return setDiff(diffSetter(time)); }, [time, showSeconds]);
    useEffect(function () { return setStatus(getStatusFromTime(diff, showSeconds)); }, [
        diff,
        showSeconds
    ]);
    useEffect(function () {
    ...
    return renderer ? (renderer({ time: time, status: status, diff: diff, format: format, text: text })) : (React.createElement("span", { id: id, className: className, style: style }, text));
}
...
export default ReactLiveTime$1;
export { setLocale };
